...
fun ClientBuilder.twitter(c: TwitterBuilderDsl.() -> Unit) {
    twitter = TwitterBuilderDsl().apply(c).build()
}

fun ClientBuilder.company(c: CompanyBuilderDsl.() -> Unit) {
    company = CompanyBuilderDsl().apply(c).build()
}
